...
window.addEventListener('DOMContentLoaded', function() {
    ...
    // Store autofilled values for change detection
    let autofilledValues = {};
    let autofilledLocations = [];

    // Listen for autofill completion to store values
    window.addEventListener('message', function(event) {
        if (event.data && event.data.type === 'autofill-complete-names') {
            // Store all practitioner fields
            autofilledValues = {
                ...
                'license-number': document.getElementById('license-number')?.value || '',
                'license-state': document.getElementById('license-state')?.value || '',
                'license-exp': document.getElementById('license-exp')?.value || ''
            };
            // Store all autofilled locations (array of objects)
            autofilledLocations = [];
            document.querySelectorAll('.location-fields').forEach((block, idx) => {
                autofilledLocations.push({
                    'practice-location': block.querySelector(`[name='practice-location${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'practice-name': block.querySelector(`[name='practice-name${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'street': block.querySelector(`[name='street${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'room': block.querySelector(`[name='room${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'city': block.querySelector(`[name='city${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'state': block.querySelector(`[name='state${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'zip': block.querySelector(`[name='zip${idx ? '-' + (idx+1) : ''}']`)?.value || '',
                    'county': block.querySelector(`[name='county${idx ? '-' + (idx+1) : ''}']`)?.value || ''
                });
            });
        }
    });

    // Intercept form submission to check for changes
    form.addEventListener('submit', function(event) {
        // Compare current values to autofilled values
        let changedFields = [];
        for (const key in autofilledValues) {
            const cur = document.getElementById(key)?.value || '';
            if (cur !== autofilledValues[key]) {
                changedFields.push({ label: key, old: autofilledValues[key], new: cur });
            }
        }
        // Compare locations
        ...
        // Check for changed or new locations
        let locationChanged = false;
        if (currentLocations.length !== autofilledLocations.length) {
            locationChanged = true;
        } else {
            for (let i = 0; i < currentLocations.length; i++) {
                for (const key in currentLocations[i]) {
                    if (currentLocations[i][key] !== (autofilledLocations[i]?.[key] || '')) {
                        locationChanged = true;
                        break;
                    }
                }
                if (locationChanged) break;
            }
        }
        if (changedFields.length > 0 || locationChanged) {
            // Show extension confirmation popup for confirmation
            extPopup.style.display = 'flex';
            const extFrame = extPopup.querySelector('iframe');
            if (extFrame) {
                extFrame.src = '../extension/confirmation.html';
                extFrame.onload = function() {
                    if (extFrame.contentWindow) {
            ...
            if (idx >= steps.length) {
                ...
                if (extFrame && extFrame.contentWindow) {
                    ...
                    try {
                        ...
                        extFrame.contentWindow.sessionStorage.setItem('autofillPracticeName', practiceNames);
                    } catch (e) {}
                    extFrame.contentWindow.postMessage({ type: 'show-autofill-complete' }, '*');
                    extFrame.contentWindow.postMessage({
                        type: 'autofill-complete-names',
                        providerName: providerName,
                        practiceName: practiceNames
                    }, '*');
                }
                return;
            }
